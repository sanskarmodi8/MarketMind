...
class DataIngestion:
    ...
    def run(self):
        ...
        """
        logger.info(f"Starting data download for {self.config.asset} from {self.config.start_date} to {self.config.end_date}")
        data = yf.download(self.config.asset, start=self.config.start_date, end=self.config.end_date)
        if data.empty:
            logger.warning(f"No data found for {self.config.asset} in the given date range.")
        else:
            data.columns = data.columns.droplevel(1)  # Drop multi-level column if exists
